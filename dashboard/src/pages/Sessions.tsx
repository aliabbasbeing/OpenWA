...
import { Plus, QrCode, RefreshCw, Trash2, Eye, Loader2, Play, Square, X, Search, Filter, Skull, PhoneOff, MessageSquare } from 'lucide-react';
...
export function Sessions() {
  ...
  const handleAutoRestartToggle = async (id: string, enabled: boolean, intervalHours?: number) => {
    try {
      const updated = await sessionApi.updateAutoRestart(id, enabled, intervalHours);
      setSessions(sessions.map(s => (s.id === id ? updated : s)));
      toast.success(
        enabled ? t('sessions.autoRestart.enabledTitle') : t('sessions.autoRestart.disabledTitle'),
        enabled ? t('sessions.autoRestart.enabled', { hours: intervalHours }) : t('sessions.autoRestart.disabled'),
      );
    } catch (err) {
      console.error('Failed to update auto-restart:', err);
      toast.error(t('sessions.autoRestart.errorTitle'), t('sessions.autoRestart.error'));
      fetchSessions();
    }
  };

  const handleCallRejectorToggle = async (id: string, enabled: boolean) => {
    try {
      const updated = await sessionApi.updateCallRejector(id, enabled);
      setSessions(sessions.map(s => (s.id === id ? updated : s)));
      toast.success(
        enabled ? t('sessions.callRejector.enabledTitle') : t('sessions.callRejector.disabledTitle'),
        enabled ? t('sessions.callRejector.enabled') : t('sessions.callRejector.disabled'),
      );
    } catch (err) {
      console.error('Failed to update call rejector:', err);
      toast.error(t('sessions.callRejector.errorTitle'), t('sessions.callRejector.error'));
      fetchSessions();
    }
  };

  const handleCallRejectorAutoReplyToggle = async (id: string, enabled: boolean) => {
    try {
      const updated = await sessionApi.updateCallRejectorAutoReply(id, enabled);
      setSessions(sessions.map(s => (s.id === id ? updated : s)));
      toast.success(
        enabled ? t('sessions.callRejectorAutoReply.enabledTitle') : t('sessions.callRejectorAutoReply.disabledTitle'),
        enabled ? t('sessions.callRejectorAutoReply.enabled') : t('sessions.callRejectorAutoReply.disabled'),
      );
    } catch (err) {
      console.error('Failed to update call rejector auto-reply:', err);
      toast.error(t('sessions.callRejectorAutoReply.errorTitle'), t('sessions.callRejectorAutoReply.error'));
      fetchSessions();
    }
  };
          ...
          filteredSessions.map(session => (
            <div key={session.id} className="session-card">
              ...
              <div className="card-actions">
                ...
                {canWrite && (
                  <button
                    className={`btn-action ${session.callRejector ? 'danger' : ''}`}
                    onClick={() => handleCallRejectorToggle(session.id, !session.callRejector)}
                  >
                    <PhoneOff size={16} />
                    {session.callRejector ? t('sessions.callRejector.on') : t('sessions.callRejector.off')}
                  </button>
                )}
                {canWrite && session.callRejector && (
                  <button
                    className={`btn-action ${session.callRejectorAutoReply ? 'success' : ''}`}
                    onClick={() => handleCallRejectorAutoReplyToggle(session.id, !session.callRejectorAutoReply)}
                  >
                    <MessageSquare size={16} />
                    {session.callRejectorAutoReply ? t('sessions.callRejectorAutoReply.on') : t('sessions.callRejectorAutoReply.off')}
                  </button>
                )}
                {canWrite && (
                  <select
                    className="auto-restart-select"
                    value={session.autoRestart ? String(session.autoRestartIntervalHours ?? 24) : ''}
                    onChange={e => {
                      const val = e.target.value;
                      if (val === '') {
                        handleAutoRestartToggle(session.id, false);
                      } else {
                        handleAutoRestartToggle(session.id, true, Number(val) as 1 | 2 | 4 | 8 | 12 | 24);
                      }
                    }}
                  >
                    <option value="">{t('sessions.autoRestart.off')}</option>
                    <option value="0.033">{t('sessions.autoRestart.test2m')}</option>
                    <option value="1">{t('sessions.autoRestart.every1h')}</option>
                    <option value="2">{t('sessions.autoRestart.every2h')}</option>
                    <option value="4">{t('sessions.autoRestart.every4h')}</option>
                    <option value="8">{t('sessions.autoRestart.every8h')}</option>
                    <option value="12">{t('sessions.autoRestart.every12h')}</option>
                    <option value="24">{t('sessions.autoRestart.every24h')}</option>
                  </select>
                )}
              </div>
            </div>
