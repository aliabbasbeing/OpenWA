...
export interface Session {
  ...
  autoRestart?: boolean;
  autoRestartIntervalHours?: number;
  autoRestartLastRun?: string | null;
  callRejector?: boolean;
  callRejectorAutoReply?: boolean;
}
...
export const sessionApi = {
  ...
  updateAutoRestart: (id: string, enabled: boolean, intervalHours?: number) =>
    request<Session>(`/sessions/${id}/auto-restart`, {
      method: 'PATCH',
      body: JSON.stringify({ enabled, intervalHours }),
    }),
  updateCallRejector: (id: string, enabled: boolean) =>
    request<Session>(`/sessions/${id}/call-rejector`, {
      method: 'PATCH',
      body: JSON.stringify({ enabled }),
    }),
  updateCallRejectorAutoReply: (id: string, enabled: boolean) =>
    request<Session>(`/sessions/${id}/call-rejector-auto-reply`, {
      method: 'PATCH',
      body: JSON.stringify({ enabled }),
    }),
...
export interface Campaign {
  ...
  contactSource: 'manual' | 'contact_list';
...
export const campaignApi = {
  ...
  update: (id: string, data: { name?: string; messageTemplate?: string; messageVariations?: string[]; settings?: Partial<CampaignSettings> }) =>
...
export const contactListApi = {
  list: () => request<ContactList[]>('/campaigns/contact-lists'),
  get: (id: string) => request<ContactList>(`/campaigns/contact-lists/${id}`),
  create: (data: { name: string; description?: string }) =>
    request<ContactList>('/campaigns/contact-lists', { method: 'POST', body: JSON.stringify(data) }),
  addContacts: (id: string, contacts: Array<{ number: string; name?: string; variables?: Record<string, string> }>) =>
    request<ContactList>(`/campaigns/contact-lists/${id}/contacts`, { method: 'POST', body: JSON.stringify({ contacts }) }),
...
export const blacklistApi = {
  list: () => request<BlacklistEntry[]>('/campaigns/blacklist'),
  add: (data: { number: string; reason?: string; sessionId?: string }) =>
    request<BlacklistEntry>('/campaigns/blacklist', { method: 'POST', body: JSON.stringify(data) }),
  import: (numbers: string[], reason?: string, sessionId?: string) =>
    request<{ imported: number; skipped: number }>('/campaigns/blacklist/import', {
      method: 'POST',
      body: JSON.stringify({ numbers, reason, sessionId }),
    }),
  check: (number: string, sessionId?: string) =>
    request<{ isBlacklisted: boolean; entry?: BlacklistEntry }>(
      `/campaigns/blacklist/check?number=${encodeURIComponent(number)}${sessionId ? `&sessionId=${sessionId}` : ''}`,
    ),
