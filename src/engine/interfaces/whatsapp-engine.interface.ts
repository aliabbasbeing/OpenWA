...
export interface EngineEventCallbacks {
  ...
  /**
   * Fired for incoming calls when the engine supports real-time call events.
   * The session service checks the session's `callRejector` flag and auto-rejects if enabled.
   */
...
export interface IWhatsAppEngine {
  ...
  // Calls
  /**
   * Reject (decline) an incoming call. Best-effort; engines that don't support call
   * rejection should no-op. `callId` is the native call identifier; `from` is the
   * caller's JID/number.
   */
