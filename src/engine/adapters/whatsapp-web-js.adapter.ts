...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  private setupEventHandlers(): void {
    ...
    this.client.on('call', async (call: any) => {
      try {
        const callId = call.id ?? call.callId ?? '';
        const from = call.peerJid ?? call.from ?? '';
        this.logger.debug(`Incoming call from ${from}, video: ${call.isVideo}`, {
          callId,
          from,
        });
        ...
        this.logger.error('Error processing incoming call', String(error));
      }
    });
  ...
  async rejectCall(callId: string, from: string): Promise<void> {
    ...
    if (!page) {
      this.logger.warn('Cannot reject call: puppeteer page not available');
      return;
    }
    if (!callId || !from) {
      this.logger.warn(`Cannot reject call: missing callId (${callId}) or from (${from})`);
      return;
    }
    this.logger.log(`Attempting to reject call ${callId} from ${from}`, { callId, from });
    try {
      await page.evaluate(
        async (peerJid: string, id: string) => {
          const userId = window.require('WAWebUserPrefsMeUser')
            .getMaybeMePnUser()._serialized;

          // Convert @lid to @s.whatsapp.net (phone number JID) — server silently drops reject stanzas
          // addressed to @lid because it can't map LID → PN internally for the call session.
          let targetJid = peerJid;
          if (peerJid.endsWith('@lid')) {
            try {
              const lidWid = window.require('WAWebWidFactory').createWid(peerJid);
              // Try multiple LID→PN resolution methods across WA Web versions
              const LidUtils = window.require('WAWebLidMigrationUtils');
              if (LidUtils?.getPnForLid) {
                const pnUser = LidUtils.getPnForLid(lidWid);
                if (pnUser?._serialized) {
                  targetJid = pnUser._serialized;
                }
              } else if (LidUtils?.toPn) {
                const pn = LidUtils.toPn(lidWid);
                if (pn?._serialized) {
                  targetJid = pn._serialized;
                }
              }
              // Fallback: check Store.LidMapping / PnLidMap
              if (targetJid === peerJid) {
                const collections = window.require('WAWebCollections');
                const lidMap = collections?.LidMapping || collections?.PnLidMap;
                if (lidMap?.get) {
                  const mapping = lidMap.get(lidWid);
                  if (mapping?.pn?._serialized) {
                    targetJid = mapping.pn._serialized;
                  }
                }
              }
              // Fallback: try contact store
              if (targetJid === peerJid) {
                try {
                  const contact = await window.require('WAWebCollections').Contact.find(lidWid);
                  if (contact?.phoneNumber?._serialized) {
                    targetJid = contact.phoneNumber._serialized;
                  } else if (contact?.phoneNumber) {
                    targetJid = String(contact.phoneNumber) + '@s.whatsapp.net';
                  }
                } catch {}
              }
            } catch {}
          ...
          // Build the JID for the stanza (use targetJid which is now @s.whatsapp.net format)
          const targetWid = window.require('WAWebWidFactory').createWid(targetJid);
          const stanzaJid = window.require('WAWebCommsWapMd').JID(targetWid);
          ...
          const stanza = window.require('WAWap').wap(
            ...
            [
              window.require('WAWap').wap('reject', {
                'call-id': id,
                'call-creator': stanzaJid,
                count: '0',
              }),
            ],
          );

          // Try Store.Socket.send first (newer builds), fallback to deprecatedCastStanza
          const socket = window.require('WAWebSocketModel')?.Socket || (window as any).Store?.Socket;
          if (socket?.send) {
            await socket.send(stanza);
          } else {
      ...
      this.logger.log(`Call rejected: ${callId} from ${from}`, { callId, from });
    } catch (error) {
      this.logger.error(`Error rejecting call ${callId}: ${String(error)}`, String(error));
    }
  }
