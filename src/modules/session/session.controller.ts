...
export class SessionController {
  ...
  @RequireRole(ApiKeyRole.OPERATOR)
  @ApiOperation({ summary: 'Configure auto-restart schedule for a session' })
  @ApiParam({ name: 'id', description: 'Session ID' })
  @ApiResponse({ status: 200, description: 'Auto-restart configured', type: SessionResponseDto })
  @ApiResponse({ status: 404, description: 'Session not found' })
  async updateAutoRestart(
    @Param('id') id: string,
    @Body() dto: UpdateAutoRestartDto,
  ): Promise<SessionResponseDto> {
    const session = await this.sessionService.updateAutoRestart(id, dto);
    await this.auditService.logInfo(AuditAction.SESSION_UPDATED, {
      sessionId: id,
      sessionName: session.name,
      metadata: { autoRestart: dto.enabled, intervalHours: dto.intervalHours },
    });
    return this.transformSession(session);
  }

  @Patch(':id/call-rejector')
  @RequireRole(ApiKeyRole.OPERATOR)
  @ApiOperation({ summary: 'Toggle call rejector for a session' })
  @ApiParam({ name: 'id', description: 'Session ID' })
  @ApiResponse({ status: 200, description: 'Call rejector toggled' })
  @ApiResponse({ status: 404, description: 'Session not found' })
  async updateCallRejector(
    @Param('id') id: string,
    @Body() dto: { enabled: boolean },
  ): Promise<SessionResponseDto> {
    const session = await this.sessionService.updateCallRejector(id, dto.enabled);
    await this.auditService.logInfo(AuditAction.SESSION_UPDATED, {
      sessionId: id,
      sessionName: session.name,
      metadata: { callRejector: dto.enabled },
    });
    return this.transformSession(session);
  }

  @Patch(':id/call-rejector-auto-reply')
  @RequireRole(ApiKeyRole.OPERATOR)
  @ApiOperation({ summary: 'Toggle call rejector auto-reply for a session' })
  @ApiParam({ name: 'id', description: 'Session ID' })
  @ApiResponse({ status: 200, description: 'Call rejector auto-reply toggled' })
  @ApiResponse({ status: 404, description: 'Session not found' })
  async updateCallRejectorAutoReply(
    @Param('id') id: string,
    @Body() dto: { enabled: boolean },
  ): Promise<SessionResponseDto> {
    const session = await this.sessionService.updateCallRejectorAutoReply(id, dto.enabled);
    await this.auditService.logInfo(AuditAction.SESSION_UPDATED, {
      sessionId: id,
      sessionName: session.name,
      metadata: { callRejectorAutoReply: dto.enabled },
    });
    return this.transformSession(session);
  }
