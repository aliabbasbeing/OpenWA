...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap {
  ...
  private async initializeEngine(id: string, session: Session): Promise<void> {
    ...
    await engine.initialize({
      ...
      onCall: (call): void => {
        if (!this.isLiveEngine(id, engine)) return;
        this.logger.debug(`Incoming call from ${call.from}, video: ${call.isVideo}`, {
          sessionId: id,
          callId: call.callId,
          from: call.from,
        ...
        // Auto-reject if callRejector is enabled on this session
        // Read fresh from DB since the captured session object may be stale
        void this.sessionRepository.findOne({ where: { id } }).then(async freshSession => {
          if (freshSession?.callRejector && typeof engine.rejectCall === 'function') {
            try {
              await engine.rejectCall(call.callId, call.from);
              this.logger.log(`Successfully rejected call from ${call.from}`, {
                sessionId: id,
                callId: call.callId,
              ...
              if (freshSession.callRejectorAutoReply) {
                try {
                  let chatId = call.from;
                  if (call.from.endsWith('@lid') && typeof engine.resolveContactPhone === 'function') {
                    const resolvedPhone = await engine.resolveContactPhone(call.from);
                    if (resolvedPhone) {
                      chatId = resolvedPhone.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
                    }
                  }
                  // Show typing indicator, then send message (simulates natural behavior)
                  if (typeof engine.sendChatState === 'function') {
                    await engine.sendChatState(chatId, 'typing');
                    await new Promise(resolve => setTimeout(resolve, 2000));
                  }
                  await engine.sendTextMessage(chatId, "I can't take your call right now. Please send me a text message and I'll get back to you.");
                  if (typeof engine.sendChatState === 'function') {
                    await engine.sendChatState(chatId, 'paused');
                  }
                  this.logger.log(`Auto-reply sent to ${chatId}`, {
                    sessionId: id,
                    action: 'call_rejector_auto_reply_sent',
                  });
                } catch (replyErr) {
                  this.logger.error(`Failed to send auto-reply: ${String(replyErr)}`, String(replyErr));
                }
              }
            } catch (err) {
              this.logger.error(`Failed to reject call: ${String(err)}`, String(err));
            }
          }
        });

        void this.webhookService.dispatch(id, 'session.call', { sessionId: id, ...call });
      },
    });
  }
  ...
  async updateAutoRestart(id: string, dto: { enabled: boolean; intervalHours?: number }): Promise<Session> {
    const session = await this.findOne(id);
    session.autoRestart = dto.enabled;
    if (dto.intervalHours !== undefined) {
      session.autoRestartIntervalHours = dto.intervalHours;
    }
    if (!dto.enabled) {
      session.autoRestartLastRun = null;
    }
    await this.sessionRepository.save(session);
    this.logger.log(`Auto-restart ${dto.enabled ? 'enabled' : 'disabled'} for session: ${session.name}`, {
      sessionId: id,
      intervalHours: dto.intervalHours,
      action: 'auto_restart_updated',
    });
    return session;
  }

  async updateCallRejector(id: string, enabled: boolean): Promise<Session> {
    const session = await this.findOne(id);
    session.callRejector = enabled;
    await this.sessionRepository.save(session);
    this.logger.log(`Call rejector ${enabled ? 'enabled' : 'disabled'} for session: ${session.name}`, {
      sessionId: id,
      action: 'call_rejector_updated',
    });
    return session;
  }

  async updateCallRejectorAutoReply(id: string, enabled: boolean): Promise<Session> {
    const session = await this.findOne(id);
    session.callRejectorAutoReply = enabled;
    await this.sessionRepository.save(session);
    this.logger.log(`Call rejector auto-reply ${enabled ? 'enabled' : 'disabled'} for session: ${session.name}`, {
      sessionId: id,
      action: 'call_rejector_auto_reply_updated',
    });
    return session;
  }
  ...
  async resolveContactPhone(id: string, contactId: string): Promise<string | null> {
    ...
    return engine.resolveContactPhone(contactId);
  }
